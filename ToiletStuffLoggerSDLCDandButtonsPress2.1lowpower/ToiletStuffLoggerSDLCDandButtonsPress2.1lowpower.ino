#define LogFileName "datalog.csv"
#define SecondsButtonPress 9
#define backlighttime 60
...
//button timing 
unsigned long timethreshold = SecondsButtonPress; // in seconds
...
void setup()   {       
  ...
  if (! RTC.isrunning()) {
   ...
   if(!SD.exists(LogFileName)) //move this to setup?
   {
    file = SD.open(LogFileName, FILE_WRITE);
    file.println("sep=;");
    file.println("PersonNr;ButtonNr;DateTime");
    file.close();
  }
...
void loop() {
  ...
  if(!digitalRead(8))  lcdbacklighttime = currenttime + backlighttime;
  
  if ( lcdbacklighttime > currenttime) lcd.backlight();
